...
class GeminiChatModel(ChatGoogleGenerativeAI):
    ...
    def _generate(self, messages: list[BaseMessage], *args: Any, **kwargs: Any) -> ChatResult:
        model_calls.count("gemini", self.model, "chat")
...
def _model(settings: Settings, model: str) -> BaseChatModel:
    return GeminiChatModel(
        ...
        # The agent calls the model through `astream`, which would bypass `_agenerate` (and so
        # the error mapping and the fallback). Replies are never streamed to the user anyway.
